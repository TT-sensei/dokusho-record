...
(function (global) {
  ...
  function checkBadgeAchievement(beforeBooks, afterBooks) {
    ...
    if (totalNew > 0 || monthlyNew > 0) {
      ...
      setTimeout(function () { U.showAchievement('🏅 バッジをゲット! ' + parts.join('・')); }, 650);
    }
  }
  ...
  function showPostRegister(bookId, beforeBooks) {
    ...
    var html = '<div class="rr-post-register">' +
      ...
      '</div><button type="button" class="rr-btn rr-btn--cta" data-post-done>本棚を見る</button></div>';
  ...
  function readFileAsDataUrl(file) {
    return new Promise(function (resolve, reject) {
      var reader = new FileReader(); reader.onload = function () { resolve(reader.result); }; reader.onerror = reject; reader.readAsDataURL(file);
    });
  }
